...
> **状态**：规范文档 · **`/photo` API 已上线**（`apps/api/data/photo/`：82 cat · 8132 张）。  
> 当前缺口：全库 meta（enriched **699 / 8132**）与 COS 原图上传，不是再写 PhotoModule。
...
| 阶段 | 执行者 | 输入 | 输出 | 你现在在哪 |
| ------ | ------ | ------ | ------ | ---------- |
| **一** | 龙虾 Agent | 本文 + `图库分类目录.json` | 本地图库完整 + 全 meta | 🚧 meta 仍缺（699 / 8132） |
| **二** | Cursor | 阶段一图库 | `apps/api/data/photo/*.json` | ✅ 运行时 manifest 已生成 |
| **三** | Cursor | manifest + COS 密钥 | CDN 可访问 `/photo` | ✅ API；🚧 原图未批量上 COS |

**阶段一成功标准**：见 §8 验收清单。  
原图上 COS 可与 meta 补全并行；缺 meta 不影响已上线的按 cat/scene 选图，但影响版权字段与质检。

---

## 2. 本地图库审计

> **2026-08 运行时**：`apps/api/data/photo/` 已有 **82 cat / 8132 张**；`enriched_meta` **699**。  
> 下表为 2026-07-03 采集审计，用于对照 meta 缺口，不再表示「API 未做」。
