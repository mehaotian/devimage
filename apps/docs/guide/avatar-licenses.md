...
DevImage 头像由**图即风格**与**三方接入**组成，许可责任按来源区分。
...
| style | 名称 | 许可 | 备注 |
| ------ | ------ | ------ | ------ |
| devimg | 图即头像 | 图即 | 主入口；`variant` / `text` / `bg` / `fg` |
| devimg-geo | 几何弧环 | 图即 | 独立几何风格 |
| devimg-pattern | 纹理 pattern | 图即 | CSS 纹理满铺；`pattern` / `shape` / `text` / `fg` |
| devimg-gradient | 渐变圆 | 图即 | `devimg` 别名 |
| devimg-mesh | 网格渐变 | 图即 | `devimg` 别名 |
| devimg-initials | 渐变首字 | 图即 | `devimg` 别名 |

图即风格无第三方署名要求；输出 SVG **不含**第三方生成注释。
