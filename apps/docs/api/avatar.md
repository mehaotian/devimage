...
Playground 分三个 Tab：**快速试玩**（默认，调参数 + 预览）、**画廊**（全部风格 + 纹理 pattern 缩略图，懒加载）、**代码参考**（HTTP / HTML 片段）。
...
当前共 **59** 种风格：**20** 种图即风格（含 3 个别名）+ **39** 种三方接入。完整 JSON 列表：`GET /avatar/styles`。

### 来源与三方库

| 来源 | engine | provider | 风格数 | 库 / 站点 | npm |
| ------ | ------ | ------ | ------: | ------ | ------ |
| 图即风格 | `native` | `devimage` | 20 | 图即 SVG 生成 | — |
...
**分组（group）**：`gradient` 渐变 · `geometric` 几何 · `text` 文字 · `pixel` 像素 · `character` 角色 · `icon` 图标 · `abstract` 抽象 · `retro` 复古 · `symbol` 符号 · `filter` 滤镜。
...
完整 JSON：`GET /avatar/patterns`；可视化浏览请用上方 Playground **「画廊」** Tab（纹理 pattern 默认折叠；含符号 7 + 精品 4）。灵感来自 [CSS3 Patterns Gallery](https://leaverou.github.io/css3patterns/)（MIT），服务端以 SVG `<pattern>` 渲染；**符号类**使用 `c1` / `c2` / `c3` 点缀，**精品类**额外使用 `c4` 四色格线。
...
纯 SVG 算法生成，无三方库与设计资源依赖。Playground **「画廊」** Tab 可按分组预览；同一 `style` + `seed` 输出 deterministic。

| 分组 | style | 名称 | 说明 | 示例 |
| ------ | ------ | ------ | ------ | ------ |
| abstract | `devimg-mandala` | 曼陀罗 | 旋转对称多层花瓣 | `/avatar/devimg-mandala/Luna/128` |
| abstract | `devimg-topo` | 等高线地形 | 伪高度场等高线 | `/avatar/devimg-topo/Luna/128` |
| abstract | `devimg-hash` | 哈希色条 | seed FNV 哈希色带可视化 | `/avatar/devimg-hash/Luna/128` |
| abstract | `devimg-matrix` | 类 QR 矩阵 | QR 视觉模块阵（**不可扫描**） | `/avatar/devimg-matrix/demo/128` |
| geometric | `devimg-bubbles` | 圆堆积 | 非重叠圆 packing | `/avatar/devimg-bubbles/Luna/128` |
| text | `devimg-radical` | 部首纹理 | 首字 Unicode → pattern + 居中首字 | `/avatar/devimg-radical/张三/128` |
| retro | `devimg-pixel` | 抽象像素 | 16×16 色块网格 | `/avatar/devimg-pixel/Luna/128` |
| filter | `devimg-glass` | 玻璃拟态 | 半透明 + blur 色块 | `/avatar/devimg-glass/Luna/128` |
| filter | `devimg-neon` | 霓虹描边 | 深底 + glow 描边 | `/avatar/devimg-neon/Luna/128` |
| filter | `devimg-paper` | 纸雕 | 多层 offset + 硬边阴影 | `/avatar/devimg-paper/Luna/128` |
| filter | `devimg-riso` | Risograph | 有限色 + 错位叠印 | `/avatar/devimg-riso/Luna/128` |
| symbol | `devimg-bot` | 极简机器人 | primitive 几何机器人 | `/avatar/devimg-bot/Luna/128` |
| symbol | `devimg-face` | 表情符号 | 圆眼 + 弧嘴 procedural 表情 | `/avatar/devimg-face/Luna/128` |
| symbol | `devimg-flower` | 花朵 | 旋转对称花瓣 | `/avatar/devimg-flower/Luna/128` |

```html
<img src="http://localhost:3000/avatar/devimg-glass/Luna/128" alt="Luna" width="128" height="128" />
...
<img src="http://localhost:3000/avatar/devimg-neon/Luna/128" />
...
| `styles[].group` | `gradient` \| `geometric` \| `text` \| `pixel` \| `character` \| `icon` \| `abstract` \| `retro` \| `symbol` \| `filter` |
| `styles[].license` | 许可标识 |
